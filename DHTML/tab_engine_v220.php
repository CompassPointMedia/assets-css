<?php
/* --------------------------- Tab Engine Version 2.20 ----------------------------------
2010-12-15
----------
forked off from "layer" engine, cleaned up so much as to justify a whole new system.
$cgrp is not used any more

2009-06-10
----------
the key(s) for the $__tabs__ array must be alphabetic
...
*****/
foreach($__tabs__ as $n=>$v){
	//get all other cg default layers that haven't been set so they can be read easily
	if(!$cgrp[$v['CGPrefix']]){
		$cgrp[$v['CGPrefix']]=$__tabs__[$n]['defaultLayer'];
	}
}
//called initially to set display of layers
if(function_exists('cg')){
	mail($developerEmail, 'Error file '.__FILE__.', line '.__LINE__,get_globals('function cg and its page called twice'),$fromHdrBugs);
}else{
...
<?php
if($tabHelpSystem){
	echo "var tabHelpSystem='$tabHelpSystem';\n\n";
	echo "//initial condition\n";
	echo "var helpSet=false;\n";
}else{
	echo "var tabHelpSystem=false;\n";
}
//declare the array for each interface
foreach ($__tabs__ as $n=>$v){
	echo "\n";
	if($v['tabSet']){
		echo "var $n= new Array();\n";
		$i=-1;
		foreach($v['tabSet'] as $w){
			if(@in_array($w, $hideTabs[$tabGroup]))continue;
			$i++;
			echo $n.'['.$i.'] = "'.$w.'";'."\n";
		}
	}
...
?>
if(typeof loadHelpPath=='undefined')var loadHelpPath='/client/help/help_01_exe.php';
